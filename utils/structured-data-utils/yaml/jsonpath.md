...
Mnemonic: Star = Select All (shape resembles a burst covering all items)
...
Minus One = Last (negative index = tail element)
...
7. Show only pods in Running state (with filter + custom columns):
...
range … end | Loop | Range = Repeat for Each
